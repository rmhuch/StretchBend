...
def run_2DDVR(dataDict):
    ...
    npz_filename = os.path.join(str(dataDict["MainDir"]), f"{name}_2D_DVR_bigGrid.npz")
    ...
    # uX = np.unique(x)
    # uY = np.unique(y)
    # fit_pot = interpolate.interp2d(uX, uY, np.reshape(pot, (len(uX), len(uY))), kind="cubic",
    #                                fill_value=Constants.convert(12000, "wavenumbers", to_AU=True))
    # newx = np.linspace(r12-Constants.convert(0.35, "angstroms", to_AU=True),
    #                    r12+Constants.convert(0.65, "angstroms", to_AU=True), 41)
    # newy = np.linspace(HOH-(44*(np.pi/180)), HOH+(44*(np.pi/180)), 41)
    # nX, nY = np.meshgrid(newx, newy)
    # xy = np.column_stack((nX.flatten(), nY.flatten()))
    # extra_pot = fit_pot(newx, newy).flatten()
...
if __name__ == '__main__':
    ...
    dat = np.load(os.path.join(MainDir, f"{name}_bigDataDict.npz"), allow_pickle=True)
    a = run_2DDVR(dat)
